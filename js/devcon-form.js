  ...
  devconform.addEventListener('submit',async function(e){
     ...
     if(userName.value && userEmail.value && userPhone_no.value){
        if (userEmail.value.match(checkValidEmail) && userPhone_no.value.length == userPhone_no.maxLength){
                ...
                const res = fetch("https://app.blinkcrm.in/api/leads", {
                method: "POST",
                mode: "cors",
                    headers: {
                    ...
                    lead: {
                        ...
                        console.log(data);
                        location.href="/thank-you-devcon-event";
                        // location.href ="https://dashboard.razorops.com/users/sign_up?utm_source=devcon-event";
